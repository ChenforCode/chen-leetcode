 ...
 * @Date 2021/12/10 10:37 上午
 * @Param
 * @Return
 * @Description 做一个词频统计即可，利用26长度的数组记录每个字符串的单词个数
 * 如果符合要求需要，自己的数组每一项大于等于要比对的字符串词频数组每一项。那么自己
 * 就是符合要求的，然后再比对长度即可
 **/
...
class ShortestCompletingWord {
    ...
    class Solution {
        public String shortestCompletingWord(String l, String[] words) {
            l = l.toLowerCase();
            String res = "12345678901234567890";
            ...
            for (int i = 0; i < words.length; i++) {
                ...
                for (int j = 0; j < curChars.length; j++) {
                    if (curChars[j] > 122 || curChars[j] < 97) {
                        continue;
                    }
                    have[curChars[j] - 97]++;
                }
