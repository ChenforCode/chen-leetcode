...
/**
 *
 * 模拟的一个思路，遍历到chars[i]，如果chars[i] == ?，那么chars[i] = char[i - 1] + 1
 * 如果chars[i] != ? && chars[i] == chars[i - 1]。chars[i - 1] = char[i - 1] + 1
 * 第二句话的原因？因为源字符串本身不可能重复，如果出现了abb这种情况，一定是第一个b是由?变成导致的，
 * 即a?b变成了abb，因此遍历到第二个b的时候，判断前边出现了一个b，因此需要把第一个b再加1，保证和第二个b
 * 不一样的同时也保证了和a不一样
 */
        ...
        public String modifyString(String s) {
            ...
            for (int i = 1; i < chars.length; i++) {
                if (chars[i] == '?') {
                    ...
                    chars[i - 1] = (char) ((chars[i - 1] - 'a' + 1) % 26 + 'a');
                }
            }
